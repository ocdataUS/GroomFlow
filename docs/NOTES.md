...
- `Plugin` now bootstraps lightweight services (`BBGF\Bootstrap\Assets_Service`, `Admin_Menu_Service`, `Rest_Service`, `Cli_Service`). Use `bbgf()->get_placeholder_board_markup()` if you need the placeholder helper—it's proxied through the assets service while keeping public APIs stable.
...
- Board previews localize strings + ARIA labels through `bbgf_board_script_settings` (JS payload) and `bbgf_placeholder_board_data` (placeholder dataset) filters; hook these to swap default timers, placeholder cards, or localization before shortcode/admin rendering.
- When the board loads in admin or via the shortcode, the PHP renderer uses the live payload from `Visit_Service::build_board_payload()`, so capacity hints and cards match the REST API before JavaScript hydration. Fall back to the filters above if you truly need alternate placeholder data.
- Board payload/stage/view lookups now use short object-cache TTLs (5s payload, 120s metadata/views) and auto-flush after CRUD; force a reset with `wp eval 'bbgf()->visit_service()->flush_cache();'` and tune via filters (`bbgf_board_payload_cache_ttl`, `bbgf_board_metadata_cache_ttl`, `bbgf_board_views_cache_ttl`) if needed.
