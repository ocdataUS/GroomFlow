...
| **S7 · Next Slice Intake** | Product Owner needs to define the next slice scope and provide a clean baseline (dev + no pending branch changes) before new implementation can begin. | Pending direction — QA plan can’t be drafted until scope is known. | Blocked |
