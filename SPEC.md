...
- **Task ritual:** Non-trivial repo work must run through Asana. Every active task should define `Goal`, `Context`, `Constraints`, and `Done When`, work on `main` by default unless Product explicitly asks for a separate branch, install the packaged ZIP in Docker, reseed demo data if needed, run `scripts/qa-phpcs plugin/bb-groomflow`, and perform the full admin happy-path (Clients, Guardians, Services, Packages, Flags, Views, Settings). Artifact paths live under `/opt/qa/artifacts/` and belong in the Asana task, `AGENT_HANDOFF.md`, and `qa/QA_LOG.md`.
